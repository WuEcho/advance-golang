...
import (
	...
	"io"
	"math/big"
	"pdx-chain-so/pkg/pdx-chain/crypto/gmsm/sm3"
	vrf2 "pdx-chain-so/pkg/pdx-chain/crypto/vrf"
)
...
func Sm2Verify(pub *PublicKey, msg, uid []byte, r, s *big.Int) bool {
	...
	//fmt.Println("uid:", hex.EncodeToString(default_uid))
	za, err := ZA(pub, uid)
	if err != nil {
		return false
	}
	//zaHex := hex.EncodeToString(za)
	//fmt.Println("zaHex:", zaHex)
	e, err := msgHash(za, msg)
	if err != nil {
		return false
	}
...
func ZA(pub *PublicKey, uid []byte) ([]byte, error) {
	...
	//fmt.Println("xA:", pub.X.String())
	//fmt.Println("yA:", pub.Y.String())
	//
	//fmt.Println("xBuf hex", hex.EncodeToString(xBuf))
	//fmt.Println("yBuf hex", hex.EncodeToString(yBuf))
	za.Write(xBuf)
	za.Write(yBuf)
...
func (pk *PublicKey) ProofToHash(m, proof []byte) (index [32]byte, err error) {
	...
	if got, want := len(proof), 64+65; got != want {
		return nilIndex, vrf2.ErrInvalidVRF
	}
...
func NewVRFSigner(key *ecdsa.PrivateKey) (vrf2.IPrivateKey, error) {
	if *(key.Params()) != *curve.Params() {
		return nil, vrf2.ErrPointNotOnCurve
	}
...
func NewVRFVerifier(pubkey *ecdsa.PublicKey) (vrf2.IPublicKey, error) {
