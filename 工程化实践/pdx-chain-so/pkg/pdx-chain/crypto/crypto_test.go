...
func TestECDH(t *testing.T) {
	//privA, _ := GenerateKey()
	//t.Logf("privA:%x", FromECDSA(privA))
	//privB, _ := GenerateKey()
	//t.Logf("privB:%x", FromECDSA(privB))

	privA, err := HexToECDSA("631615bca552c907f48c9955b7fb31a4af674a992668a59357c17bf78543f503")
	if err != nil {
		t.Fatal(err)
	}
	t.Logf("privA:%x", FromECDSA(privA))
	privB, err := HexToECDSA("12a34b53915de488f6d2ccb3f638ba014589387d8c2c871d69c605e4c8e48162")
	if err != nil {
		t.Fatal(err)
	}
	t.Logf("privB:%x", FromECDSA(privB))
	...
	b, _ := pubA.Curve.ScalarMult(pubA.X, pubA.Y, privB.D.Bytes())
	a, _ := pubA.Curve.ScalarMult(pubB.X, pubB.Y, privA.D.Bytes())

	shared1 := sha256.Sum256(b.Bytes())
	shared2 := sha256.Sum256(a.Bytes())

	t.Logf("shared1 key Bob:%x", shared1)
	t.Logf("shared2 key Bob:%x", shared2)

	orig := "tonysu"
	encryptCode := AesEncrypt(orig, shared1[:])
	t.Log("密文：" , encryptCode)

	decryptCode := AesDecrypt(encryptCode, shared2[:])
	t.Log("解密结果：", decryptCode)

}

func AesEncrypt(orig string, k []byte) string {
	// 转成字节数组
	origData := []byte(orig)
	//k := []byte(key)
